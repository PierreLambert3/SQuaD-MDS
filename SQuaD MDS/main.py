...
hparams = {
    'metric': 'euclidean',       # other option: "relative rbf distance" --> see the code of SQuaD_MDS() in optimsers.py or the paper for a description
    'n iter': 1000,              # 1000 is plenty if initialised with PCA
    ...
    'exaggerate D': True,        # use squared distances for the first part of the optimisation
    'stop exaggeration': 0.6   # when to stop the exaggeration in terms of percentage of 'n iter'
}
Xld_SQuaD_MDS = Xld.copy()
tic = time.time()
SQuaD_MDS(hparams, Xhd, Xld_SQuaD_MDS)
...
tic = time.time()
SQuaD_MDS(hparams, Xhd, Xld_SQuaD_MDS_rbf)
...
from sklearn.decomposition import PCA
tic = time.time()
Xld_SMACOF = smacof(pairwise_distances(Xhd), n_components=2, init=Xld.copy(), n_init=1)[0]
time_SMACOF = time.time() - tic
print('SMACOF MDS (with PCA initialiation) done in ', np.round(time_SMACOF,3), 's')
...
print('hybrid SQuaD_MDS + light tSNE done in ', np.round(time_SQuaD_MDS_light_tSNE,3), 's')
...
print('hybrid SQuaD_MDS + strong tSNE done in ', np.round(time_SQuaD_MDS_strong_tSNE,3), 's')
...
# ~~~~~~~~~~~~~~~~~~ AUC of Rnx(K) curves ~~~~~~~~~~~~~~~~~~
_, AUC_light_tSNE  = eval_dr_quality_Rnx(pairwise_distances(Xhd), pairwise_distances(Xld_SQuaD_MDS_light_tSNE))
_, AUC_strong_tSNE = eval_dr_quality_Rnx(pairwise_distances(Xhd), pairwise_distances(Xld_SQuaD_MDS_strong_tSNE))
_, AUC_tSNE        = eval_dr_quality_Rnx(pairwise_distances(Xhd), pairwise_distances(Xld_tsne))
...
print("hybrid SQuaD MDS + light tSNE  : " + str(D_corr_hybrid_SQuaD_MDS_light_tSNE) + '   (Rnx(K) AUC : ',str(AUC_light_tSNE),')')
print("hybrid SQuaD MDS + strong tSNE : " + str(D_corr_hybrid_SQuaD_MDS_strong_tSNE)+ '   (Rnx(K) AUC : ',str(AUC_strong_tSNE),')')
print("t-SNE                          : " + str(D_corr_tSNE)+ '   (Rnx(K) AUC : ',str(AUC_tSNE),')')
...
subs[1, 1].scatter(Xld_SQuaD_MDS_light_tSNE[:,0], Xld_SQuaD_MDS_light_tSNE[:,1], c=Y, s=1)
subs[1, 1].set_title("SQuaD MDS + light t-SNE")
subs[1, 2].scatter(Xld_SQuaD_MDS_strong_tSNE[:,0], Xld_SQuaD_MDS_strong_tSNE[:,1], c=Y, s=1)
